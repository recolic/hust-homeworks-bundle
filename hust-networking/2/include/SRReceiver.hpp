#ifndef _RNETW_SR_RECV_HPP_
#define _RNETW_SR_RECV_HPP_ 1
...
class SRReceiver final : public RdtReceiver {
    using pkt_seq_t = decltype(Packet::seqnum);
public:
    SRReceiver(const pkt_seq_t windowSize) : windowSize(windowSize), cache(windowSize) {
    ...
    virtual ~SRReceiver() override = default;
...
private:
    ...
    void do_send_ack(pkt_seq_t seq) const {
        Packet ackPkt;
        ackPkt.acknum = ackPkt.seqnum = seq;
